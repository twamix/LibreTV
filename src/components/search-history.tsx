 ...
 * - 外框取 accent 色，与输入框的聚焦边框同色，避免拼接处断色；
 * - 左右内边距与输入框文字对齐；作为浮层不占文档流。
 */
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between pl-4 pr-2 py-2 border-b border-line">
        ...
        <button
          type="button"
          className="px-2 py-1 rounded-md text-xs text-faint hover:text-red-400 hover:bg-chip transition-colors"
          onClick={onClearAll}
        >
          清空
        </button>
      ...
      >
        {matches.map((h, i) => (
          ...
          >
            ...
            <button
              type="button"
              className="shrink-0 p-1.5 mr-2 rounded-md text-faint hover:text-red-400 hover:bg-chip transition-colors"
